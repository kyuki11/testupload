              ...
              <!--PAGE CONTENT ENDS-->
                    ...
                    <div class="widget-body">
                      <div class="widget-main no-padding">
                                           ...
                                           <?php echo $this->session->flashdata('save_pengajar'); ?>
                                            <form action="<?php echo base_url();?>index.php/admin/simpan_pengajar" method="post">
                                              <table width="60%" border="0" cellspacing="0" cellpadding="0" class="table table-striped table-bordered table-hover">
                                              <?php
                        foreach ($pengajar->result_array() as $m)
                        {
                        ?>
                    ...
                    <td width="23%" align="right">Kode Mata Pelajaran</td>
                      <td width="6%" align="center">:</td>
                      <td width="71%"><label for="kd_mapel"></label>
                        <label>
                          <select name="kd_mapel" id="kd_mapel">
                            <option value=""><?php echo $m ['kd_mapel']; ?></option>
                            <?php
                            foreach ($mapel->result_array() as $d) {
                            ?>
                              <option value="<?php echo $d['kd_mapel']; ?>"><?php echo $d['kd_mapel']; ?></option>
                            <?php
                            }
                            ?>
                          </select>
                    ...
                    <tr>
                      ...
                      <td><label for="mapel"></label>
                        <label>
                          <input id="mapel" name="mapel" type="text" placeholder="" class="form-control input-md" value="<?php echo $m['mapel']; ?>" required="">
                        </label>
                      </td>
                                                ...
                                                <tr>
                                                  <td>&nbsp;</td>
                                                  <td>&nbsp;</td>
                                                  <td><button class="btn btn-small btn-primary"><i class="icon-download-alt"></i> Simpan Data</button> <a href="<?php echo base_url();?>index.php/admin/pengajar" class="btn btn-danger btn-small"> <i class="icon-external-link"></i>Kembali</a>
    ...
    <!-- script guru -->
        ...
        <script>
          $(document).ready(function() {
            ...
            //yang dilakukan ketika isi input kd_mapel berubah
            $('#kd_mapel').on('change', function() {

              const url_get_all_mapel = $('#content').data('url') + '/get_all_mapel'
              $.ajax({
                url: url_get_all_mapel,
                type: 'POST',
                dataType: 'json',
                data: {
                  kd_mapel: $(this).val()
                ...
                success: function(data) {
                  $('input[name="mapel"]').val(data.mapel).prop('readonly', true)
